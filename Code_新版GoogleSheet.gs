...
const LIST_CACHE_SECONDS = 180;
const SETTINGS_CACHE_SECONDS = 180;
const LIST_CACHE_VERSION_KEY = "LIST_CACHE_VERSION";
const LIST_CACHE_VERSION_CACHE_KEY = "list-cache-version";
...
function listInventory_(params) {
  ...
  const cacheVersion = getListCacheVersion_();
  ...
  if (cached) {
    try {
      ...
      console.warn("列表快取解析失敗，改讀工作表", err);
    }
  }

  const payload = readInventoryRowsForList_();
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    if (rowStatus.indexOf("\u51fa\u8ca8\u5b8c\u6210") !== -1) continue;
    ...
    const createdAt = row[index.createdAt];
    const updatedAt = row[index.updatedAt];
    records.push({
      _sortTime: sort === "created" ? toTime_(createdAt) : toTime_(updatedAt || createdAt),
      row: payload.firstRow + i,
  ...
  const items = records.slice(start, start + pageSize).map(stripInternalRecordFields_);
  const result = {
    ok: true,
  ...
  try {
    cache.put(cacheKey, JSON.stringify(result), LIST_CACHE_SECONDS);
  } catch (err) {
    console.warn("列表快取寫入失敗", err);
  }
  return result;
}
...
function getSettings_() {
  const cache = CacheService.getScriptCache();
  const cacheKey = "settings|" + API_VERSION;
  const cached = cache.get(cacheKey);
  if (cached) {
    try {
      return JSON.parse(cached);
    } catch (err) {
      console.warn("設定快取解析失敗，改讀工作表", err);
    }
  }
  ...
  const values = lastRow ? sheet.getRange(1, 1, lastRow, 2).getValues() : [];
  let headerRow = 0;
  ...
  const result = { ok: true, apiVersion: API_VERSION, partners, statuses };
  try {
    cache.put(cacheKey, JSON.stringify(result), SETTINGS_CACHE_SECONDS);
  } catch (err) {
    console.warn("設定快取寫入失敗", err);
  }
  return result;
}
...
function readInventoryRowsForList_() {
  const sheet = getSheet_(SHEET_NAMES.inventory, INVENTORY_HEADERS);
  const headerRow = getHeaderRow_(sheet, INVENTORY_HEADERS);
  const lastRow = sheet.getLastRow();
  const headerColumns = Math.max(sheet.getLastColumn(), INVENTORY_HEADERS.length);
  const header = sheet.getRange(headerRow, 1, 1, headerColumns).getValues()[0];
  const index = headerIndex_(header);
  const maxHeaderIndex = INVENTORY_HEADERS.reduce(function(max, headerName) {
    return Math.max(max, Number(index[headerName] || 0));
  }, 0);
  const readColumns = Math.max(maxHeaderIndex + 1, INVENTORY_HEADERS.length);
  const rowCount = Math.max(0, lastRow - headerRow);
  const rows = rowCount ? sheet.getRange(headerRow + 1, 1, rowCount, readColumns).getValues() : [];
...
function stripInternalRecordFields_(record) {
  const output = {};
  Object.keys(record).forEach(function(key) {
    if (key.charAt(0) !== "_") output[key] = record[key];
  });
  return output;
}
...
function getListCacheVersion_() {
  const cache = CacheService.getScriptCache();
  const cached = cache.get(LIST_CACHE_VERSION_CACHE_KEY);
  if (cached) return cached;

  const properties = PropertiesService.getScriptProperties();
  const saved = properties.getProperty(LIST_CACHE_VERSION_KEY) || "1";
  cache.put(LIST_CACHE_VERSION_CACHE_KEY, saved, LIST_CACHE_SECONDS);
  return saved;
}

function clearListCache_() {
  const nextVersion = String(Date.now());
  PropertiesService.getScriptProperties().setProperty(LIST_CACHE_VERSION_KEY, nextVersion);
  CacheService.getScriptCache().put(LIST_CACHE_VERSION_CACHE_KEY, nextVersion, LIST_CACHE_SECONDS);
}
